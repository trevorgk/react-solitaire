...
@connect(state => ({klondike: state.klondike.data}), dispatch => bindActionCreators(klondikeActions, dispatch))
export default class KlondikeCard extends Component {
  ...
  renderOverlay (color) {
    return <div style={{
      position: 'absolute', top: 0, left: 0, height: '100%', width: '100%', zIndex: 1, opacity: 0.5, backgroundColor: color
    }}></div>;}

  displayCard (card) {
    if (!card.show) {return PlayingCards.Card.backFace;}
    let objCard = new PlayingCards.Card(card.suit, card.rank);
    return objCard.getImageFile();
  }

  isActive (active, card) {
    if(!active || !card.show) {return false;}
